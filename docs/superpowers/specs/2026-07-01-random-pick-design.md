...
- 選ばれたゲームは既存の `GameModal` で表示する（編集・削除もそのまま使える）
- 未プレイのゲームが0件のとき、ボタンは disabled にする
- 選出時に「選んでいます…」風の演出（ガチャ感）を入れる。演出中はパラパラとタイトルが切り替わり、最後に確定したゲームのモーダルが開く
...
| `src/pages/GameListPage.jsx` | `isPicking`・`pickingTitle` の state、`handleRandomPick` 関数、`RandomPickOverlay` の条件付きレンダーを追加 |
